...
if st.session_state.get('analyze_clicked', False):
    sequence = st.session_state.get('current_seq', sequence)
    seq_id = st.session_state.get('current_seq_id', seq_id)
    ...
    if valid_len < 3:
        ...
        # Check if we need to build the cached items
        if st.session_state.get('last_built_sequence') != sequence:
            ...
            st.session_state['tree_seq'] = tree_seq
